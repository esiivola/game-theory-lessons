...
Numbering matches the shipped course: 64 core lessons (L1 to L64) across 17 units, plus a 12-case
applied track (A1 to A12). The communication arc separates adverse selection and screening (L30),
cheap talk (L31), and Bayesian persuasion (L32). `src/content/lessons/<slug>.mdx` remains the source
of record for lesson content.
...
The build uses separate widgets per lesson for the auction and economic-game labs (`AuctionSandbox`,
`WinnersCurse`, `OptimalReserve`, `VCG`, `PositionAuction`; `Ultimatum`, `PublicGoods`,
`PublicGoodsPunishment`, `Reciprocity`).
...
### L3 Preferences and expected utility
...
### L4 Dominant and dominated strategies
...
### L7 The Prisoner's Dilemma
- **Example game:** two gas stations choose Hold or Cut with the standard payoffs R=3, T=5, P=1, S=0. Cutting strictly dominates holding, so both cut for 1 each even though both holding would pay 3 each. Real-world framing: a price war, with the 1914 Christmas truce as the opening contrast.
- **Interactive:** play the gas-station matrix against a bot after predicting whether both firms hold or cut. The selected cell reports both payoffs, and the reveal connects the outcome to strict dominance.
- **Worked example:** compare Hold and Cut against each rival action: 5>3 when the rival holds and 1>0 when it cuts. Repeat for both players to show that (Cut, Cut) is the unique Nash equilibrium.
- **Misconceptions:** the jointly best outcome must be Nash; a bad equilibrium means somebody acted irrationally; the extra repeated-game condition $2R>T+S$ is part of the one-shot definition.
- **Source:** Flood (1958); Axelrod (1984); Osborne and Rubinstein (1994).

### L8 Classic 2x2 games
...
- **Worked example:** classify Stag Hunt by finding both pure Nash and ranking them (both prefer Stag-Stag but Hare-Hare is safer); contrast Matching Pennies where best-response marks never coincide (motivates L9).
...
### L9 Mixed strategies and mixed Nash
...
### L10 Existence of Nash equilibrium
...
### L11 Zero-sum games and minimax
- **Example game:** a 2x2 zero-sum matrix (payoffs to row): T/L 2, T/R -1, B/L -1, B/R 1; value 1/5, optimal mixes p=q=2/5. Plus RPS. Real-world framing: penalty kicks and attack/defend allocation. Include the "minimax in the field" reality-check callout (professionals really do randomise, forward-link to L63).
...
### L12 Schelling and focal points
...
### L15 Extensive form and backward induction
...
- **Misconceptions:** bargaining always splits 50/50; a better outside option means I concede more (I get more); the Nash solution describes real haggling (it is axiomatic; L20 is the strategic model).
- **Source:** Nash (1950).

### L20 Strategic bargaining
...
### L21 Repeated games and discounting
...
- **Misconceptions:** repetition always produces cooperation (needs δ >= threshold); a finitely repeated PD sustains cooperation (backward induction unravels it); grim trigger is robust to noise (it is not, motivates L23).
...
### L23 Repeated games with imperfect monitoring
...
### L25 Bayesian games and Bayes-Nash
...
### L26 Auctions: first and second price
...
### L28 Perfect Bayesian equilibrium
...
### L30 Adverse selection and screening
...
### L32 Information design and Bayesian persuasion
...
### L33 Contract theory and moral hazard
...
### L34 Sequential equilibrium and refinements
- **Example game:** the 2x2 (T,L)=(1,1), else (0,0): both (T,L) and (B,R) are Nash, but (B,R) survives only on weakly dominated play. Selten's Horse distinguishes sequential from PBE. Reference the intuitive criterion from L29. Real-world framing: discarding equilibria propped up by never-tested threats.
...
### L37 Social choice: Arrow and Gibbard-Satterthwaite
...
### L38 Mechanism design and the revelation principle
...
### L41 Implementation and Myerson-Satterthwaite
...
### L42 Auctions III: position auctions
...
### L43 Coalitional games and the core
...
### L48 School choice and design without money
...
### L49 Evolutionarily stable strategies
...
### L50 Replicator dynamics
- **Example game:** Rock-Paper-Scissors, zero-sum; the replicator equation has interior fixed point (1/3,1/3,1/3) with closed orbits; a tie bonus of +0.1 spirals inward (stable), -0.1 outward. A Hawk-Dove toggle converges to the L49 ESS (the bridge). Real-world framing: cyclic strategy fads.
...
### L51 Stochastic stability and conventions
...
### L53 The complexity of equilibria
...
### L54 Congestion and potential games
...
### L55 Price of anarchy and Braess's paradox
...
### L56 Network and graphical games
...
### L58 Quantal response equilibrium
...
### L59 Social preferences: fairness
...
### L60 Reciprocity and intentions
...
### L61 Cooperation and punishment
...
### L63 Field tests and boundaries
...
### L64 Culture, context, and design
...
### A1 Matching markets (inline after L47) [medical match, school choice, kidney exchange]
...
- **Applies:** L46 to L48 (matching, DA, TTC, strategy-proofness).
...
### A2 Auctions in the wild (inline after L42) [spectrum, ad auctions]
...
- **Applies:** L15, L16, L21.
- **Interactive:** compare the static 10% mixed-Nash rate with a disaster-risk commitment chosen before the rival responds. A slider shows the rival's expected payoff from holding and the point where yielding becomes its best response.
- **Worked example:** in the static game, $-10p + 1(1-p)=-p$ gives the 10% symmetric mixing rate. Under commitment, the rival gets $1-11q$ from holding and $-1$ from yielding, so a disaster risk of $q=2/11$, about 18%, deters it. These are different objects.
...
### A4 Entry deterrence in business (inline after L16)
...
### A5 Co-opetition and platforms (inline after L12)
...
- **Applies:** L8 (coordination), L12.
- **Interactive:** set a price to each side and compare symmetric demand with an asymmetric case where users pull developers and bring extra revenue. The same subsidy loses money in the symmetric model but can beat balanced pricing under the displayed asymmetry.
- **Worked example:** test prices of -10 and 70. The symmetric model shows the cross-side spillover but lower profit; the asymmetric model adds stronger attraction and revenue on the paying side, making the subsidy profitable. Cross-side effects alone do not prove a subsidy is optimal.
...
### A6 The roots of cooperation (inline after L22) [Axelrod, cartels]
...
### A7 Bargaining and negotiation (inline after L20) [strikes, BATNA]
...
### A8 Mixed strategies in sport (inline after L11) [penalty kicks]
- **Scenario:** applied minimax in penalty kicks, tennis serves, and pitch selection; the equilibrium requires unpredictable mixing at frequencies that equalize success. Differentiate from L63: here the question is "how to win", play vs an adaptive exploiting bot.
- **Applies:** L9, L11.
...
- **Worked example:** with the L63 scoring matrix, the kicker's equilibrium mix is ~36% Left; at that mix the keeper cannot raise the save rate by favoring either side; "always natural side" lets an adaptive keeper cut your scoring.
...
### A9 The commons (inline after L21) [Ostrom, climate]
...
### A11 Contracts and the law (inline after L33) [moral hazard, settlement]
...
- **Applies:** L9, L16, L25, and L58 (QRE).
